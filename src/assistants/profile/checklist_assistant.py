...
from src.utils import TEXT_CURSOR, get_llm_response, stream_static_text
...
class ChecklistAssistant(Assistant):
        ...
        if user_message:
            ...
            if not self.init_message_sent:
                self.add_assistant_message(
                    "Let's get started with our first question: What Australian location are you concerned about for bushfire preparedness or resilience?",
                    thread_id,
                )
                self.init_message_sent = True
                stream_static_text(self.config["init_message"])
                return self.config["init_message"], None, []
            return super().get_assistant_response(thread_id=thread_id, allow_tools=allow_tools)
    ...
    def checklist_update(self, checklist: str):
        ...
        stream_static_text(
            f"I'd like to take a moment to formulate a few focused follow-up questions so I can tailor the bushfire resilience plan to your situation. Please don't feel any pressure if you're unsure about some answers. Please do not respond yet ...{TEXT_CURSOR}"
        )
        ...
        updated_checklist = get_llm_response(
            messages=[follow_up, {"role": "system", "content": self.config["format_instructions"]}],
        )
    ...
    def checklist_complete(self, checklist: str):
        return (
            "Checklist completed in the current in-memory session. Continue with the BushfireReadyGPT report workflow; "
